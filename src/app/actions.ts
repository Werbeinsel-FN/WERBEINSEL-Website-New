...
export async function sendContact(_prev: FormState, formData: FormData): Promise<FormState> {
  try {
    ...
    if (!parsed.success) {
      return {
        ok: false,
        message: 'Bitte füllen Sie die markierten Pflichtfelder aus.',
    ...
    return { ok: true, message: 'Danke! Ihre Anfrage ist bei uns eingegangen. Wir melden uns in der Regel innerhalb von 24 Stunden.' }
  } catch (err) {
...
export async function sendApplication(_prev: FormState, formData: FormData): Promise<FormState> {
  try {
    const raw = {
      ...
      verfuegbarAb: formData.get('verfuegbarAb'),
      services: formData.getAll('services'),
    ...
    if (!(file instanceof File) || file.size === 0) {
      return {
        ok: false,
        message: 'Bitte füllen Sie die markierten Pflichtfelder aus.',
        errors: { datei: ['Bitte eine Datei hochladen.'] },
      }
    }
    ...
    if (!typeOk) {
      return {
        ok: false,
        errors: { datei: ['Erlaubt: PDF, DOC, DOCX, JPG, PNG oder ZIP.'] },
      }
    }
    ...
    return {
      ok: true,
      message: 'Danke! Ihre Bewerbung ist bei uns eingegangen. Wir melden uns zeitnah.',
    }
  } catch (err) {
    console.error('[sendApplication]', err)
    return {
      ok: false,
      message: 'Etwas ist schiefgelaufen. Bitte später erneut versuchen.',
    }
