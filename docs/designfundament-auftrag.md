...
Tailwind-Klassen: `brand-yellow`, `brand-black`, `brand-white`, `brand-card-dark`, `brand-card-light`, `brand-muted`, `brand-subtle`, `brand-on-light`, `brand-divider`, `brand-control`, `brand-control-hover`, `brand-error` (z. B. `bg-brand-control`, `text-brand-on-light`, `fill-brand-yellow`).
...
- Leistungsstruktur (drei statt fünf Leistungen) – inzwischen umgesetzt: Plakatwerbung, Folierung & Beschriftung, Foto & Video; die Seiten Grafikdesign und Social Media sind entfernt.
...
- **npm-audit-Hinweise** aus `npm ci` prüfen und beheben. **Erledigt:** next 16.3.6, payload 3.90.2, sharp 0.35.4, vitest 4.1.11 und `npm audit fix`. Offen bleiben 5 mittlere Hinweise zu `esbuild` über `drizzle-kit` in `@payloadcms/db-postgres`; sie betreffen nur den esbuild-Entwicklungsserver, der hier nicht läuft, und haben keine Behebung.
- **Test `tests/int/api.int.spec.ts` schlägt lokal fehl:** Ohne `DATABASE_URL` versucht Payload eine Postgres-Verbindung zu `localhost:5432` („cannot connect to Postgres“). `npm run test:int` läuft deshalb lokal nicht vollständig durch. Test so anpassen, dass er ohne Datenbank übersprungen wird oder eine eigene Testdatenbank nutzt – nie die Produktionsdatenbank. In der CI läuft er als nicht blockierender Schritt; danach blockierend machen. **Erledigt:** Ohne `DATABASE_URL` wird der Test übersprungen, bei einem anderen Host als `localhost` bricht er ab. In der CI läuft er blockierend gegen einen eigenen Postgres-Container.
...
- **Vorlagen-Test `tests/e2e/frontend.e2e.spec.ts`:** stammt unverändert aus der Payload-Vorlage und erwartet den Titel „Payload Blank Template“; schlägt deshalb fehl. `admin.e2e.spec.ts` legt einen Testnutzer über Payload an und braucht eine Datenbank. Beide ersetzen oder entfernen. **Erledigt:** `frontend.e2e.spec.ts` ist durch einen Grundtest aller Seiten aus `pages.json` ersetzt; `admin.e2e.spec.ts` und `tests/helpers/` sind entfernt.
- **Payload-Migrationen einführen (vor dem Umzug zu AWS):** Das Projekt hat keine Migrationen (`src/migrations` fehlt, `prodMigrations` ist nicht eingerichtet). Payload gleicht das Schema nur außerhalb von `NODE_ENV=production` automatisch ab („push“); auf Vercel passiert beim Start nichts. Schemaänderungen durch Payload-Updates müssen deshalb bisher von Hand per SQL in Neon nachgezogen werden, zuletzt beim Update auf 3.90.2 die Spalte `users.reset_password_requested_at`. Aufgabe: eine Ausgangsmigration für den bestehenden Stand anlegen und in der Produktionsdatenbank als ausgeführt markieren, danach jede Schemaänderung als Migration mit `payload migrate:create` erzeugen und beim Deployment mit `payload migrate` oder `prodMigrations` anwenden. Vorher prüfen, ob die Vercel-Vorschau eine eigene Datenbank bekommen soll: `DATABASE_URL` gilt bisher für Vorschau und Produktion gemeinsam. Mit den Migrationen aufräumen, was seit der Umstellung auf drei Leistungen übrig ist: die Referenz-Kategorien `grafik` und `social` (Enum in der Datenbank) und die Block-Typen „Was wir gestalten“, „Ein Motiv“ und „Online-Kampagnen“, falls sie nicht mehr gebraucht werden. Außerdem die seit dem Entfernen von WhatsApp verwaisten Spalten löschen: `einstellungen.whatsapp`, `kontakt_seite.cards_whatsapp_label` und `kontakt_seite.cards_whatsapp_cta` (die Felder sind aus der Konfiguration entfernt, die Spalten stehen noch in der Produktionsdatenbank).
  - **Fehlende Spalte nach dem Update auf 3.90.2 (September 2026):** Seit 3.90 legt `@payloadcms/plugin-cloud-storage` (über das Vercel-Blob-Plugin) in `media` die Spalte `_objectkey` (varchar, darf leer sein) an. Sie fehlte in der Produktion („column "_objectkey" does not exist“), Löschen im Admin schlug fehl und die Leistungsseiten fielen auf den Seed zurück. Ursache: Der Schema-Vergleich lief lokal ohne `BLOB_READ_WRITE_TOKEN`, dadurch war das Plugin inaktiv und die Spalte tauchte nicht auf. Behebung per Hand-SQL in Neon: `ALTER TABLE "media" ADD COLUMN IF NOT EXISTS "_objectkey" varchar;`
  - **Lehre für jeden Schema-Vergleich:** `payload generate:db-schema` immer mit derselben Plugin-Konfiguration wie in der Produktion laufen lassen, also mit einem Platzhalter-Token im richtigen Format (`BLOB_READ_WRITE_TOKEN=vercel_blob_rw_<id>_<zufall>`), ohne `DATABASE_URL`. Heute ist das Blob-Plugin die einzige Stelle, an der die Konfiguration von einer Umgebungsvariablen abhängt; kommt eine weitere hinzu, muss sie beim Vergleich ebenfalls gesetzt sein. Mit echten Migrationen fällt dieser Handvergleich weg.
